...
import "./SafeRetrieveRandom.sol";

contract SafeLottery is Ownable {
    ...
    function enter() public payable {
        ...
        if (players.length >= quorum) revealBlock = block.number;
    }
    ...
    function revealWinner()
        ...
        payable
    {
        require(players.length >= quorum, "This lottery round is still open");
        // Ensures that the lottery's outcome can not be determined in the same block as the final ticket sale
        // This protects against contract-based and node-based attacks.
        // Block producers need to mine 3 blocks in a row to undo their commitment and prevent a losing transaction
        // from being included in the chain
        require(block.number + 2 >= revealBlock, "Revealed the result too early");
         
        uint256 indexWinner = randGen.getRandomFromRange(revealBlock, 0, players.length - 1);
